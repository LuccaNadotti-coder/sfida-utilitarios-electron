...
const m = /export const ESQUEMA = `([\s\S]*?)`;/.exec(fuente);
if (!m) throw new Error('No se pudo leer ESQUEMA de src/main/db/esquema.ts');
...
const ARTICULOS = [
  ...
  ['LIM-0003', 'JABÓN LÍQUIDO', LIM, 'GAL', 10],
  ['LIM-0004', 'DETERGENTE EN POLVO', LIM, 'KG', 10],
  ['LIM-0005', 'LEJÍA', LIM, 'GAL', 12],
...
const ing = db.prepare(
  'INSERT INTO ingresos(tipo_doc,nro_documento,fecha,proveedor,observacion) VALUES (?,?,?,?,?)',
);
...
const COMPRAS = [
  ...
  ['FACTURA', 'F001-0088', hace(95), 'COMERCIAL SUR', [['OFI-0010', 25, 16.0], ['LIM-0003', 40, 22.0], ['LIM-0005', 60, 9.5]]],
...
for (const [tipo, nro, fecha, prov, lineas] of COMPRAS) {
  const id = ing.run(tipo, nro, fecha, prov, '').lastInsertRowid;
  for (const [cod, cant, costo] of lineas) ingDet.run(id, ids[cod], cant, costo);
}

const sal = db.prepare(
  'INSERT INTO salidas(nro_vale,fecha,sucursal_id,entregado_por,recibido_por,observacion) VALUES (?,?,?,?,?,?)',
);
const salDet = db.prepare('INSERT INTO salida_det(salida_id,articulo_id,cantidad) VALUES (?,?,?)');

const anio = new Date().getFullYear();
const REPARTOS = [
  [`V${anio}-0001`, hace(88), 'SUC01', 'CARLOS RAMIREZ', 'ANA QUISPE', '', [['OFI-0001', 120], ['OFI-0010', 8], ['LIM-0001', 30]]],
  [`V${anio}-0002`, hace(70), 'SUC02', 'CARLOS RAMIREZ', 'LUIS RAMOS', '', [['OFI-0002', 90], ['LIM-0005', 12], ['LIM-0008', 40]]],
  [`V${anio}-0003`, hace(45), 'SUC03', 'CARLOS RAMIREZ', 'MARIA FERNANDEZ', 'ENTREGA CORRESPONDIENTE A LA SEMANA 34', [['OFI-0001', 80], ['LIM-0003', 5], ['LIM-0001', 25]]],
  [`V${anio}-0004`, hace(28), 'SUC01', 'CARLOS RAMIREZ', 'ANA QUISPE', '', [['OFI-0004', 100], ['OFI-0008', 40], ['LIM-0004', 20]]],
  [`V${anio}-0005`, hace(15), 'SUC02', 'CARLOS RAMIREZ', 'LUIS RAMOS', '', [['OFI-0005', 60], ['LIM-0015', 24], ['LIM-0010', 8]]],
  [`V${anio}-0006`, hace(6), 'SUC03', 'CARLOS RAMIREZ', 'MARIA FERNANDEZ', 'REPOSICIÓN QUINCENAL', [['OFI-0010', 3], ['OFI-0012', 15], ['LIM-0003', 5], ['LIM-0008', 60], ['OFI-0013', 2]]],
  [`V${anio}-0007`, hace(2), 'SUC01', 'CARLOS RAMIREZ', 'ANA QUISPE', '', [['OFI-0014', 12], ['LIM-0005', 20], ['OFI-0002', 60]]],
...
for (const [vale, fecha, cod, entrega, recibe, obs, lineas] of REPARTOS) {
  ...
  for (const [c, cant] of lineas) salDet.run(id, ids[c], cant);
}
